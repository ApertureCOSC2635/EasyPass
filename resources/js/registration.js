	...
	function checkForm() {
		...
		if (user == null) {
        	...
        	button.setAttribute("onclick","return checkQuestions();");
        	login.setAttribute("class","hidden");
        	questions.removeAttribute("class","hidden");
        	return false;
		}
		else {
			if (magicNumber != user.stored_magic) {
               displayError("Error: Email/date of birth combination does not match.", dob);
				return false;
            }
